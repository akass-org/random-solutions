...
|[start_app.sh](./niri/appManager/start_app.sh)|开机应用自启动，可设置启动的应用的目标显示器和目标工作区，以及屏蔽一些窗口如 discord 的 start 窗口|添加到开机运行的脚本，需要启动的应用列表在 [applist.txt](./niri/appManager/applist.txt) 配置，格式为：`应用 .desktop 文件名称:显示器名称:工作区名称:屏蔽标题`，不需要配置的项目请置空|